"""
Módulo para exportar dados em Excel para Django
Adaptado do export_excel.py original
"""
    ...
    def export_company_expenses(self, company_name: str, company_code: str, 
        ...
        try:
            ...
            for expense in expenses:
                ...
                ws.cell(row=row, column=4).value = float(expense.get('amount', 0))
                ws.cell(row=row, column=5).value = expense.get('created_by', 'N/A')
                ws.cell(row=row, column=6).value = expense.get('notes', '')
                ws.cell(row=row, column=7).value = str(expense.get('created_at', ''))
            ...
            if expenses:
                ...
                ws.cell(row=total_row, column=4).value = sum(float(e.get('amount', 0)) for e in expenses)
            ...
            # Importar settings para obter MEDIA_ROOT
